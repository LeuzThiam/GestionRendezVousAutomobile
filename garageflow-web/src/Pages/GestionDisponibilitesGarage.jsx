...
function GestionDisponibilitesGarage() {
  ...
  const handleSlotSubmit = async (event) => {
    ...
    try {
      ...
      setError(null);
      if (editingSlotId) {
        const updated = await updateGarageDisponibiliteRequest(editingSlotId, slotForm);
        setDisponibilites((current) => current.map((item) => (item.id === updated.id ? updated : item)));
        setMessage('Disponibilite mise a jour.');
      } else {
        const created = await createGarageDisponibiliteRequest(slotForm);
  ...
  const handleClosureSubmit = async (event) => {
    ...
    try {
      ...
      setError(null);
      if (editingClosureId) {
        const updated = await updateGarageFermetureRequest(editingClosureId, closureForm);
        setFermetures((current) => current.map((item) => (item.id === updated.id ? updated : item)));
        setMessage('Fermeture exceptionnelle mise a jour.');
      } else {
        const created = await createGarageFermetureRequest(closureForm);
  ...
  const handleDeleteClosure = async (id) => {
    try {
      ...
      setMessage(null);
      setError(null);
      await deleteGarageFermetureRequest(id);
