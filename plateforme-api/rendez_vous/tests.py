...
class RendezVousApiTests(APITestCase):
    def setUp(self):
        self.garage = Organization.objects.create(
        ...
        self.second_garage = Organization.objects.create(
        ...
        self.mecanicien.profile.role = 'employe'
        ...
        self.other_mecanicien.profile.role = 'employe'
        self.other_mecanicien.profile.garage = self.second_garage
        self.other_mecanicien.profile.save()

        cat_g1 = CategoriePrestation.objects.get(garage=self.garage, slug='entretien')
        cat_g2 = CategoriePrestation.objects.get(garage=self.second_garage, slug='entretien')
    ...
    def test_client_can_create_rendezvous(self):
        self.authenticate(self.client_user)

        response = self.client.post(
            '/api/rendezvous/',
            {
                'organization': self.garage.id,
    ...
    def test_mecanicien_cannot_create_rendezvous(self):
        ...
        response = self.client.post(
            '/api/rendezvous/',
            {
                'employe': self.other_mecanicien.id,
                'organization': self.second_garage.id,
                'date': self.build_local_datetime(days=2, hour=10).isoformat(),
                'description': 'Test',
            },
            format='json',
    ...
    def test_owner_counter_proposal_is_recorded_in_history_with_internal_note(self):
        ...
        response = self.client.patch(
            f'/api/rendezvous/{rdv.id}/',
            {
                ...
                'internal_note': 'Le creneau client n etait pas tenable pour l atelier.',
            },
            format='json',
        )
    ...
    def test_client_can_create_rendezvous_for_any_active_garage(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data['organization'], self.second_garage.id)
        self.assertEqual(response.data['client'], self.client_user.id)
        self.assertIsNone(response.data['employe'])
