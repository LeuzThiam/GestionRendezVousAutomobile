...
class AuthUserSerializer(serializers.ModelSerializer):
    ...
    organization_name = serializers.CharField(source='profile.garage.name', read_only=True)
...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    ...
    def get_token(cls, user):
        ...
        token['organization_id'] = getattr(user.profile, 'garage_id', None)
        return token

    def validate(self, attrs):
        data = super().validate(attrs)
        data['role'] = getattr(self.user.profile, 'role', 'client')
        data['organization_id'] = getattr(self.user.profile, 'garage_id', None)
        return data


class AuthOwnerRegisterSerializer(OrganizationRegistrationSerializer):
    def validate(self, attrs):
