...
import GestionRendezVousPro from '../../features/rendezvous/pages/GestionRendezVousProPage';
...
function renderPage() {
  return render(
    <MemoryRouter>
      <GestionRendezVousPro />
    </MemoryRouter>
  );
}

describe('GestionRendezVousPro', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    fetchRendezVousRequest.mockResolvedValue([
      {
        ...
        organization_name: 'Organisation centrale',
        mecanicien: null,
        service: 7,
  ...
  it('propose un meilleur choix de mecanicien sur une demande en attente', async () => {
    ...
    expect(await screen.findByText(/Meilleur choix suggéré/i)).toBeInTheDocument();
    expect(screen.getAllByText(/Sara Disponible/i).length).toBeGreaterThan(0);
    expect(screen.getAllByText(/Spécialité cohérente/i).length).toBeGreaterThan(0);
  });
